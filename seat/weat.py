...
def run_test(encs, n_samples, parametric=False):
    ...
    # First convert all keys to ints to facilitate array lookups {"0": [], "1": [], ...}
    (X, Y) = convert_keys_to_ints(X, Y)
    (A, B) = convert_keys_to_ints(A, B)
    
    # XY contains X and Y, AB contains A and B
    XY = X.copy()
    XY.update(Y)
    AB = A.copy()
    AB.update(B)

    # Create cosine similarities look up matrix shape = len(XY), len(AB)
    print("Computing cosine similarities...")
    cossims = construct_cossim_lookup(XY, AB)


    print("Null hypothesis: no difference between {} and {} in association to attributes {} and {}".format(
