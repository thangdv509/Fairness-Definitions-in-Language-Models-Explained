...
def run_experiment():
    print("------------Medium-sized LMs: Intrinsic bias - Probability-based bias - AULA------------")
    ...
    for i in range(0, 2):
        ...
        all_ranks = []
        if i == 0:
            print("------------ AULA - Crows-Pairs Dataset------------")
            inputs = load_crows_pairs_dataset()
            dataset_name = "Crows-Pairs"
        else:
            print("------------ AULA - Stereoset Dataset------------")
    ...
    df.to_csv("medium_sized/intrinsic_bias/probability_based/pseudo_log_likelihood_metrics/aula/result.csv", index=False)